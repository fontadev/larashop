...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('coupons.update', $coupon) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select"
                                ...
                                <option value="percentage"
                                    {{ old('type', $coupon->type) === 'percentage' ? 'selected' : '' }}>Percentual (%)
                                </option>
                                <option value="fixed"
                                    {{ old('type', $coupon->type) === 'fixed' ? 'selected' : '' }}>Valor Fixo (R$)</option>
                            </select>
                        </div>
                        ...
                        <button type="submit"
                            class="btn btn-primary">Atualizar Cupom</button>
                    </form>
                </div>
            </div>
        </div>
